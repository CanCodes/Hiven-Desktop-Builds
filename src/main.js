...
function createHivenClient() {
  ...
  // ScreenShare Feature
  hivenClient.webContents.session.setPermissionCheckHandler(
    async (webContents, permission, details) => {
      return true;
    }
  );
  hivenClient.webContents.session.setPermissionRequestHandler(
  ...
  hivenClient.webContents.on("new-window", async function (e, url) {
    ...
    if (url.includes("hiven.house/") || url.includes("hiven.io/invites/")) {
      let key = await hivenClient.webContents.executeJavaScript(
        "localStorage.getItem('hiven-auth')",
        true
      );
      ...
      await request.get(link, (a, b, c) => {
        ...
        request.post(
          ...
          (e, r, body) => {
            return hivenClient.loadURL(
              `https://canary.hiven.io/houses/${house_id}`
            );
          }
        );
      });
...
autoUpdater.on("update-downloaded", (info) => {
  loadingScreen.webContents.executeJavaScript(
    `updateText('Installing version ${info.version}')`
  );
...
require("electron").ipcMain.on("nativeLinkCommand", (_, name) => {
  switch (name) {
    ...
    case "maximize":
      hivenClient.isMaximized()
        ? hivenClient.unmaximize()
        : hivenClient.maximize();
      break;
  }
});
